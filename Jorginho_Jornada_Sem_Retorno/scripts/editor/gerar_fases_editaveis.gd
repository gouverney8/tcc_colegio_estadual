...
var fases := [
	{
		...
		"thin":[Vector2i(0,4),Vector2i(1,4),Vector2i(2,4)], "top":[Vector2i(0,0),Vector2i(1,0),Vector2i(2,0)], "body":[Vector2i(0,2),Vector2i(1,2),Vector2i(2,2)],
...
func _initialize() -> void:
	...
	for index in fases.size():
		gerar_fase(fases[index],floori(float(index)/2.0))
...
func gerar_fase(dados: Dictionary, familia: int) -> void:
	var root := Node2D.new()
	...
	mapa.z_index = 2
	mapa.tile_set = load(TILESETS_V2[familia]) as TileSet
